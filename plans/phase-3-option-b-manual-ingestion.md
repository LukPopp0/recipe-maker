...
- [x] Write `server/src/services/ai/prompts/manual-ingestion.ts`, following the exact structure of `server/src/services/ai/prompts/url-ingestion.ts` (same `CANONICAL_RECIPE_SHAPE` block, same pantry/tag-vocabulary embedding), with these differences:
...
- [x] Write `.test.ts`: asserts required instruction fragments are present (pantry list embedded, tag vocabulary embedded, 600-char step-description instruction present, "do not reorder" instruction present, `"manual"` source_type instruction present); asserts the output shape description does **not** contain `"main_image"`.
